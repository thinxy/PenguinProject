...
module.exports = {
    name: 'transações',
    aliases: ['tr', 'transações'],
    desc: 'Verifica as ultimas 1.000 transações de um usuário.',
    uso: '[usuário]',
    cooldown: 5500,
    run: async (client, message, args, config, database) => {
        ...
        page = parseInt(page)
        let data = doc.actions.transactions.reverse()
        ...
        const embed = (transactions) => new EmbedBuilder()
            ...
            .setTitle(`${config.money.emoji} **${config.text.separator}** Transações de \`${user.tag}\` (${page}/${parseInt((data.length / 10) + 1)})`)
        ...
        let msg = await message?.reply({ content: message.author.toString(), embeds: [embed(`${transactions ? transactions : `Nenhuma transação foi feita...`}`)], components: [buttons(svolt, sprox)] })
